...
| The signed transaction is broadcast without the operator's intent | Resolved by elimination, permanently, not just "not yet possible": [ADR-0007](adr/0007-no-broadcast-capability.md) (v0.2.8) decided `eth_sendRawTransaction` will never be added to this codebase — broadcasting a signed transaction stays the operator's own tooling. `POST /v1/mode`'s `LivePreflight` gate (v0.2.6) still refuses to arm `Live` unless a fresh-address `transfer` check passes for every `[chain]` symbol, checked fresh on every arm attempt — but that gates a mode *flag*, since no order-placing path exists behind it either. Reconciling a transaction the operator sent themselves (`sherwood-reconcile`, v0.2.8) reads a receipt only |
