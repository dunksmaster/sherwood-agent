...
| The signed transaction is broadcast without the operator's intent | Not yet possible — no code path calls `eth_sendRawTransaction`. When one exists (`sherwood-dex` + live mode), it inherits `allow_live` + admin + explicit toggle, plus the mandatory pre-flight — **the arming gate is built (v0.2.6)**: `POST /v1/mode` refuses to switch to `Live` unless a `LivePreflight` is configured and its fresh-address `transfer` check passes for every `[chain]` symbol, checked fresh on every arm attempt, not just once at startup |
