...
| `sherwood-server` | axum on loopback (non-loopback bind refused). One `{code,message,correlation_id}` error envelope. Bearer-token auth — `ApiToken` (constant-time compare via `subtle`, zeroised, generated into the vault on first run). `GET /v1/health`; `POST /v1/hook/pretooluse` runs `HookGate` against a caller-supplied portfolio/market context — a *denied* call is a `200` body, only a malformed request is `4xx`. `sherwood serve <config>` with `[server]` + `[hook]`. 10 unit tests. | RBAC roles + PAPER/LIVE toggle + kill-switch endpoint (S9b); WebSocket feed, `/metrics`, OpenAPI, rate limiting (S9c); the server holding its own live portfolio (S11–S12) |
...
120 tests pass. `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`,
