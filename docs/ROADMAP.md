...
| v0.2.1b | `sherwood-chain` — Uniswap v4 pool reads → Stock Token price → `PriceFeed` | next: `StateView.getSlot0` on a discovered pool, `sqrtPriceX96` → `Decimal` price, a polling `PriceFeed`. Addresses + recipe verified in [ROBINHOOD-CHAIN.md](ROBINHOOD-CHAIN.md). Supersedes S14b's quote loader. |
