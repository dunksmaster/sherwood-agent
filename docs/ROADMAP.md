...
| S9d | WebSocket event feed (bridges `sherwood-events`) + `utoipa`-generated OpenAPI | `server` | deferred — both want the run loop folded into the server (S11) and a stable route set. Static-file serving split out to S10.1 (done). |
