...
| v0.2.3 | `sherwood-wallets` — multi-wallet registry, per-strategy binding, spend ceilings | **done** — named `Wallet`s (a `sherwood-signer` key + symbol allowlist + a per-wallet tx-count/notional/duration budget mirroring the server's session budget), `WalletRegistry::load` (all-or-nothing), `sherwood wallets config.toml` CLI, `[[wallets]]` config. Still no RPC client, no broadcast. `wallet_for_symbol` is called from `sherwood run` as of the runner integration below. |
| v0.2.4 | `sherwood-dex` — Uniswap v4 quote + swap construction on Robinhood Chain | **done** — `ExactInputSingleSwap::execute_calldata`, Permit2 approvals, `slippage`/`deadline`/`minOut`. A word-by-word diff against a real successful transaction caught one bug (the `hookData` offset, fixed — see v0.2.4's Fixed entry in [CHANGELOG.md](../CHANGELOG.md)); `sherwood dex-simulate` now returns success for a real NVDA/USDG swap. See [`crates/dex/README.md`](../crates/dex/README.md). `sherwood dex-simulate` for the exact pool you intend to trade remains the mandatory pre-sign gate. |
| v0.2.5 | `sherwood-router` — AMM vs RFQ selection by notional | **done** — `Router::choose(notional) -> RouteChoice { venue, reason }`, pure decision logic (no RPC, no calldata, no broadcast). `Venue::{Amm, Rfq}`; below the configured `rfq_min_notional` → AMM, at/above → RFQ **iff** an RFQ venue is configured. No RFQ venue is verified on Robinhood Chain, so `rfq_available` is `false` in every real config and everything routes to the AMM today; the threshold path exists so enabling RFQ later is config, not control flow. `sherwood route <notional> [rfq_min_notional] [rfq_available]` CLI. 8 unit tests. `Router::choose` is called from `sherwood run` as of the runner integration below. See [`crates/router/README.md`](../crates/router/README.md). |
| v0.2.6 | live-mode pre-flight — refuse to arm if `rhc-probe`'s fresh-address `transfer` sim stops passing | **done** — `sherwood-server` gains a `LivePreflight` trait; `POST /v1/mode` calls it on every attempt to arm `Live`, before the mode flag flips, and fails closed (`allow_live = true` with no pre-flight wired up still refuses). `sherwood serve`'s `ChainLivePreflight` runs `sherwood_chain::probe::check_transfer_open` against every `[chain] symbols` entry plus `denom`; any token that isn't `Verdict::Permissionless`, or an RPC failure, or no symbols configured, refuses with a reason in the response body. Guards against an implementation upgrade adding an allowlist. |
| v0.2.7 | runner integration — wire `sherwood-router` + `sherwood-wallets` into `sherwood run`'s order path | **done** — new `[router]` config section. When `[[wallets]]` are configured, every paper fill now calls `Router::choose(notional)` and `WalletRegistry::wallet_for_symbol` and logs which venue and wallet a live order would have used. Logging only: no calldata is built (that needs a live pool lookup — `sherwood dex-simulate`, still a separate, explicit, manual command) and nothing is signed or sent. `sherwood run` behaves exactly as before with no `[[wallets]]` configured. |
| v0.2.8 | order reconciliation, and a permanent decision on broadcast | **done** — [ADR-0007](adr/0007-no-broadcast-capability.md): `eth_sendRawTransaction` will not be added to this codebase, ever — broadcasting a signed transaction stays the operator's own tooling, permanently, not a future TODO. Instead: `sherwood-chain` gains one new read, `EvmClient::get_transaction_receipt`; new crate `sherwood-reconcile` polls it and reconciles the outcome against the trade the operator expected, producing a `sherwood_core::Fill` on success. `sherwood reconcile <config> <tx_hash> <symbol> <buy\|sell> <qty> <price> [fee]` records it exactly like a paper fill (appended fill, updated portfolio snapshot, audit row) when `[general] state_path` is configured. Still no send capability anywhere in this codebase. See [`crates/reconcile/README.md`](../crates/reconcile/README.md). |
| v0.2.9 | server API for the v0.2 stack — `POST /v1/route`, `POST /v1/dex/simulate` | **done** — `sherwood-server` gains `POST /v1/route` (`sherwood-router`'s venue choice for a notional — pure, no RPC) and `POST /v1/dex/simulate` (wraps the same build-and-`eth_call` logic as `sherwood dex-simulate`, factored into `sherwood-cli::dex_preview` and shared by both). Both operator role. `sherwood-wallets` is deliberately **not** wired into the server — see the [2026-09-13 decision log entry](DECISIONS.md#2026-09-13). `sherwood run`/`sherwood serve` still cannot move funds under any config. |
| v0.2.10 | reconcile endpoint — `POST /v1/reconcile` | **done** — `sherwood-server` gains `POST /v1/reconcile` (operator), the same shape as v0.2.9's routes: wraps `sherwood-reconcile`'s `wait_and_reconcile`, factored into `sherwood-cli::reconcile_preview`. Reads a receipt for a tx hash the operator already broadcast — never sends anything. A `confirmed` outcome records the fill (appended fill, updated portfolio snapshot, audit row) only when `[general] state_path` is configured **and** a portfolio snapshot already exists; the response's `recorded`/`note` fields say which. Verified live against a real, already-mined transaction: `not_found`/`confirmed` states, and both the no-snapshot and successful-recording paths, all against the real chain. |
| v0.2.11 | dashboard — cash-over-time view | **done** — `frontend/` gains `CashCurve`: a dependency-free inline-SVG sparkline reconstructing cash movement from the fills already fetched for the activity feed, anchored to the live `/v1/portfolio` cash figure. No backend change and no new endpoint — purely client-derived from data the dashboard already polls. Deliberately labelled "cash over time," not "equity" — there's no mark-to-market, and the series only covers the fetched activity window, not full history. |
| v0.2.12 | `sherwood-config` — extract `AppConfig` into its own crate | **done** — `AppConfig`/every `*Section` struct/`validate()` moved out of `sherwood-cli` into a new, deliberately dependency-light `sherwood-config` crate, so `sherwood-server` can depend on the same types (needed for Phase 3's config editor) without a `server -> config -> server` cycle. `ServerSection::to_opts()` and `WalletEntry::to_core()` stayed behind in `sherwood-cli::config_ext` — both would have recreated a cycle or leaked vault/signer code into the server process; see [`crates/config/README.md`](../crates/config/README.md). Pure refactor: all 19 existing config tests moved and pass unchanged, plus a live `sherwood check`/`sherwood serve` smoke test confirmed no behavior change. |
| v0.2.13 | config editor — `GET`/`POST /v1/config` | **done** — `sherwood-server` gains `GET /v1/config` (the exact current `config.toml` text, admin role) and `POST /v1/config` (admin + body re-auth: validates a full replacement file with `sherwood-config`'s `AppConfig::validate` before writing anything, then applies it exactly like the existing `POST /v1/config/reload`). Reads/writes raw TOML text rather than round-tripping through a parsed struct, so a human-edited file's comments and formatting survive. Writes atomically (temp file + rename). `sherwood-cli::config_store::FileConfigStore` is the concrete implementation, wired unconditionally in `sherwood serve`. 7 new `sherwood-server` route tests. Verified live: a config with `mode = "live"` is rejected (`422`, file untouched), and a real `approval_mode` change is written to disk, applied live (confirmed via `GET /v1/approvals`), and readable back through `GET /v1/config`. |
| v0.2.14 | dashboard — router / dex-simulate / reconcile views | **done** — `frontend/` gains three new cards (`ChainTools.tsx`): Router (notional → AMM/RFQ), DEX simulate (`eth_call`-preview a swap), and Reconcile (read a receipt for an already-broadcast tx hash) — each a self-contained form hitting the matching v0.2.9/v0.2.10 operator endpoint. Resolves the open question from the Phase 2–5 plan: these are **not** hidden by role in the UI — the dashboard has no client-side role detection (no `/v1/whoami`), and no existing card hides itself by role either (`Controls.tsx`'s admin actions are equally visible to every token); a wrong-role token gets the same 403-through-the-error-envelope treatment as any other failed call. 4 new Playwright specs. Verified live against the real chain: `POST /v1/route` for a real notional renders the actual AMM/RFQ decision in the card. |
