...
| v0.2.3 | `sherwood-wallets` — multi-wallet registry, per-strategy binding, spend ceilings | **done** — named `Wallet`s (a `sherwood-signer` key + symbol allowlist + a per-wallet tx-count/notional/duration budget mirroring the server's session budget), `WalletRegistry::load` (all-or-nothing), `sherwood wallets config.toml` CLI, `[[wallets]]` config. Still no RPC client, no broadcast; nothing calls `wallet_for_symbol` yet. |
| v0.2.4 | `sherwood-dex` — Uniswap v4 quote + swap construction on Robinhood Chain | **built, partly verified** — `ExactInputSingleSwap::execute_calldata`, Permit2 approvals, `slippage`/`deadline`/`minOut`. Encoding confirmed byte-for-byte against a real successful transaction; a live `sherwood dex-simulate` against the actual NVDA/USDG pool still reverts (balance + approvals confirmed fine) — root cause open, see [`crates/dex/README.md`](../crates/dex/README.md). Do not sign/broadcast until that simulation succeeds. |
| v0.2.5 | `sherwood-router` — AMM vs RFQ selection by notional | **done** — `Router::choose(notional) -> RouteChoice { venue, reason }`, pure decision logic (no RPC, no calldata, no broadcast). `Venue::{Amm, Rfq}`; below the configured `rfq_min_notional` → AMM, at/above → RFQ **iff** an RFQ venue is configured. No RFQ venue is verified on Robinhood Chain, so `rfq_available` is `false` in every real config and everything routes to the AMM today; the threshold path exists so enabling RFQ later is config, not control flow. `sherwood route <notional> [rfq_min_notional] [rfq_available]` CLI. 8 unit tests. See [`crates/router/README.md`](../crates/router/README.md). |
