...
| v0.2.1c | wire `sherwood-chain` into `sherwood_core::PriceFeed` + `sherwood run`/`backtest` config | next: a polling feed (the `PriceFeed` trait is sync; the feed does its async reads via `block_in_place` + `Handle::block_on`, matching how a live feed is expected to bridge into the sync loop per `feed.rs`'s doc comment). Supersedes S14b's quote loader. |
