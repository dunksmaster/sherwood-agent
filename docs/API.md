...
| `POST` | `/v1/reconcile` | operator | `{ tx_hash, symbol, side: "buy"\|"sell", qty, price, fee? }` → `{ status: "not_found"\|"reverted"\|"confirmed", block_number?, gas_used?, recorded, note? }`. Reads a receipt for a transaction the operator already broadcast with their own tooling ([ADR-0007](adr/0007-no-broadcast-capability.md)) — never sends anything. A `confirmed` outcome records the fill (appended fill, updated portfolio snapshot, audit row) only if `[general] state_path` is configured **and** a portfolio snapshot already exists; `recorded` and `note` say which. `404` if the server has no `[chain]` endpoint configured for this. |
| `GET` | `/v1/config` | admin | `{ toml: "<the exact current config.toml text>" }`. Every secret field in `AppConfig` is a `vault:` reference, never a literal (`AppConfig::validate` enforces that on write), so this never returns a credential. `404` if the server has no config store configured for this. |
| `POST` | `/v1/config` | admin | `{ toml: "<a complete replacement config.toml>", reauth: "<admin token>" }`. Validates the whole file with `sherwood-config`'s `AppConfig::validate` before writing anything — a `422` on failure leaves the file on disk untouched. On success, writes it and applies it exactly like `POST /v1/config/reload` (same response shape, plus `written: true`). `404` if the server has no config store configured for this. |
