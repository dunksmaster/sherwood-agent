//! TOML config loading and validation for sherwood-agent. Mirrors
//! `config.example.toml`.
//!
//! Extracted from `sherwood-cli` (v0.2.12) so `sherwood-server` can depend on
//! these types directly for a config-editor endpoint, without a
//! `server -> config -> server` cycle. Two conversions stay behind in
//! `sherwood-cli` instead of moving here — see `Cargo.toml`'s comment for
//! why: `ServerSection::to_opts() -> sherwood_server::ServerOpts` (a hard
//! cycle) and `WalletEntry::to_core() -> sherwood_wallets::WalletConfig`
//! (cycle-safe, but would transitively pull vault/signer code into
//! `sherwood-server`, undoing the 2026-09-13 decision to keep that out of
...
impl Default for ChainSection {
    fn default() -> Self {
        Self {
            enabled: false,
            // Duplicated from `sherwood_chain::tokens::DEFAULT_RPC` rather
            // than taking a dependency on sherwood-chain for one constant —
            // that crate pulls in reqwest/tokio, disproportionate weight for
            // this otherwise dependency-light crate. Keep these in sync.
            rpc_url: "https://rpc.mainnet.chain.robinhood.com".to_owned(),
            symbols: Vec::new(),
            denom: "USDG".into(),
...
///
/// Note: unlike the other sections, this one has no `to_opts()` here — the
/// runtime knobs `sherwood-server` needs (`ServerOpts`) are assembled in
/// `sherwood-cli`, which is the only crate allowed to depend on both this
/// crate and `sherwood-server`.
#[derive(Debug, Deserialize)]
#[serde(default)]
...
///
/// Note: no `to_core()` here — see this crate's `Cargo.toml` for why
/// `WalletEntry -> sherwood_wallets::WalletConfig` stays in `sherwood-cli`.
